<script>
  ...
  async function get_scores() {
    const db_res = await fetch(
      `https://whispering-tor-80065.herokuapp.com/api/get/scores?uuid=${user.uuid.toString()}`
    );
            ...
            <Row>
              {#if !attmp.isCorrect}
                <div class="card-score">
                  <span class="red-text"
                    >You answered: {attmp.answered.replace(/["]+/g, "")}</span
                  >
                  <br />
                  <br />
                  <span class="green-text"
                    >Correct Answer: {attmp.correct.replace(/["]+/g, "")}</span
                  >
                </div>
              {:else}
                <div class="card-score">
                  <span class="green-text"
                    >Correct Answer: {attmp.answered.replace(/["]+/g, "")}</span
                  >
                </div>
              {/if}
            </Row>
